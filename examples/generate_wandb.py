...
# 生成启动sweep的脚本
def main(params):
    src_dir, project_name, dataset_names, model_names, folds, save_dir_suffix, all_dir, launch_file, generate_all, emb_types,train_ratio, greed_number, int_ratio, pos = params["src_dir"], params["project_name"], params["dataset_names"], \
    params["model_names"], params["folds"], params["save_dir_suffix"], params["all_dir"], params["launch_file"], params["generate_all"], params["emb_types"], params["train_ratio"], params["greed_number"], params["int_ratio"], params["position"]
    ...
    with open("../configs/wandb.json") as fin,\
        ...
        #                     data = data.replace("[\"dkt\"]", "[\"" + m + "\"]")
                            ...
                            data = data.replace('[1.0]', str([train_ratio]))
                            data = data.replace('[0]', str([greed_number]))
                            data = data.replace('[50]', str([int_ratio]))
                            data = data.replace('["left"]', str([pos]))
                            data = data.replace('BATCH_SIZE',str(params["batch_size"]))
                            data = data.replace('[1]',str([params["k1"]]))
                            data = data.replace('[3]',str([params["k2"]]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--int_ratio", type=int, default=50)
