...
def save_config(train_config, model_config, data_config, params, save_dir, args=None):
    ...
    if args:
        d = {"train_config": train_config, 'model_config': model_config,
             "data_config": data_config, "params": params, "train_args": vars(args)}
    else:
        d = {"train_config": train_config, 'model_config': model_config,
             "data_config": data_config, "params": params}
...
def main(params, args=None):
    ...
    with open("../configs/kt_config.json") as f:
        ...
        if model_name in ["dkvmn", "deep_irt", "sakt", "saint", "saint++", "akt", "atkt", "lpkt", "skvmn", "gnn4kt", "simplekt", "dkt"]:
            train_config["batch_size"] = 4  # because of OOM
        if model_name in ["akt_conv"]:
        ...
        if model_name in ["dkt", 'iekt', 'simplekt', 'akt']:
            for key in ["model_name", "dataset_name", "emb_type", "save_dir", "fold", "seed", 'train_ratio', 'greed_number', "int_ratio","position"]:
    ...
    batch_size, num_epochs, optimizer = train_config[
        "batch_size"], train_config["num_epochs"], train_config["optimizer"]

    with open("../configs/data_config.json") as fin:
        data_config = json.load(fin)
    # prefer to use the maxlen in data config1
    ...
    if model_name not in ["simplekt_sr", "parkt"]:
        train_loader, valid_loader, curtrain = init_dataset4train(
            dataset_name, model_name, emb_type, data_config, fold, batch_size, args)
    ...
    if model_name in ["stosakt"]:
        save_config(train_config, model_config,
                    data_config[dataset_name], params, ckpt_path, args)
    else:
        save_config(train_config, model_config,
                    data_config[dataset_name], params, ckpt_path)
    ...
    if model_name in ["parkt"]:
        dpath = os.path.join(
            data_config[dataset_name]["dpath"], "keyid2idx.json")
        with open(dpath, "r") as f:
            map_json = json.load(f)
            num_stu = len(map_json["uid"])
        model = init_model(model_name, model_config,
                           data_config[dataset_name], emb_type, args, num_stu)
        print(
            f"model_parameter:{sum(dict((p.data_ptr(), p.numel()) for p in model.parameters()).values())}")
    else:
        model = init_model(model_name, model_config,
                           data_config[dataset_name], emb_type, args)
        print(
            f"model_parameter:{sum(dict((p.data_ptr(), p.numel()) for p in model.parameters()).values())}")
    ...
    if model_name == "hawkes":
        ...
        opt = torch.optim.Adam(optdict, lr=learning_rate,
    ...
    if emb_type.find("cl") != -1:
        ...
        testauc, testacc, window_testauc, window_testacc, validauc, validacc, best_epoch = train_model(
            model, train_loader, valid_loader, num_epochs, opt, ckpt_path, None, None, save_model, dataset_name, fold, curtrain=curtrain, batch_size=batch_size)
    else:
        testauc, testacc, window_testauc, window_testacc, validauc, validacc, best_epoch = train_model(
            model, train_loader, valid_loader, num_epochs, opt, ckpt_path, None, None, save_model, dataset_name, fold, use_wandb=use_wandb)
    ...
    if save_model:
        if model_name not in ["parkt"]:
            ...
            best_model = init_model(
                model_name, model_config, data_config[dataset_name], emb_type, args, num_stu)
    ...
    print(str(fold) + "\t" + model_name + "\t" + emb_type + "\t" + str(round(testauc, 4)) + "\t" + str(round(testacc, 4)) + "\t" +
          str(round(window_testauc, 4)) + "\t" + str(round(window_testacc, 4)) + "\t" + str(validauc) + "\t" + str(validacc) + "\t" + str(best_epoch))
