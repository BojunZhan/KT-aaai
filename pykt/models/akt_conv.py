...
class AKTConv(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks, dropout, d_ff=256, 
            kq_same=1, final_fc_dim=512, num_attn_heads=8, separate_qa=False, l2=1e-5, k1=1, k2=3, emb_type="qid", emb_path="", pretrain_dim=768):
        super().__init__()
        """
        Input:
        ...
        self.model = Architecture(n_question=n_question, n_blocks=n_blocks, n_heads=num_attn_heads, dropout=dropout,
                                    d_model=d_model, d_feature=d_model / num_attn_heads, d_ff=d_ff,  kq_same=self.kq_same, k1=k1, k2=k2, model_type=self.model_type, emb_type=self.emb_type)
...
class Architecture(nn.Module):
    def __init__(self, n_question,  n_blocks, d_model, d_feature,
                 d_ff, n_heads, dropout, kq_same, k1, k2, model_type, emb_type):
        super().__init__()
        """
        ...
        if model_type in {'akt_conv'}:
            self.blocks_1 = nn.ModuleList([
                TransformerLayer(d_model=d_model, d_feature=d_model // n_heads,
                                 d_ff=d_ff, dropout=dropout, n_heads=n_heads, kq_same=kq_same, k1=k1, k2=k2, emb_type=emb_type)
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, d_feature,
                 d_ff, n_heads, dropout, k1, k2,  kq_same, emb_type):
        super().__init__()
        """
        ...
        self.icb = ICB(in_features=d_model, hidden_features=d_ff, kernel_size1=k1, kernel_size2=k2)
...
class CausalConv1d(nn.Conv1d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dilation=1, **kwargs):
        ...
        super().__init__(in_channels, out_channels, kernel_size, stride=stride, padding=self.causal_padding, dilation=dilation, **kwargs)
...
class ICB(nn.Module):
    def __init__(self, in_features, hidden_features, kernel_size1=1, kernel_size2=3, dropout=0):
        super().__init__()
        ...
        self.conv3 = CausalConv1d(hidden_features, in_features, kernel_size1)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x):
        ...
        # print(f"x1.shape: {x1.shape}, x1_1.shape: {x1_1.shape}, x1_2.shape: {x1_2.shape}")
        
        x2 = self.conv2(x)
        x2_1 = self.activation(x2)
        x2_2 = self.dropout(x2_1)
        # print(f"x2.shape: {x2.shape},x2_1.shape: {x2_1.shape}, x2_2.shape: {x2_2.shape}")
        out_1 = x1 * x2_2
        out_2 = x2 * x1_2
        x = self.conv3(out_1 + out_2)
