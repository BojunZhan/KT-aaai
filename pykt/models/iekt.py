...
class IEKTNet(nn.Module): 
    def __init__(self, num_q,num_c,emb_size,max_concepts,lamb=40,n_layer=1,cog_levels=10,acq_levels=10,dropout=0,gamma=0.93, emb_type='qc_merge', emb_path="", pretrain_dim=768,device='cpu'):
        ...
        self.cog_matrix = nn.Parameter(torch.randn(cog_levels, emb_size * 2).to(device), requires_grad=True) 
        self.acq_matrix = nn.Parameter(torch.randn(acq_levels, emb_size * 2).to(device), requires_grad=True)
        ...
        self.prob_emb = nn.Parameter(torch.randn(num_q, emb_size).to(device), requires_grad=True)#题目表征
        self.gamma = gamma
        self.lamb = lamb
        self.gru_h = mygru(0, emb_size * 4, emb_size)
        self.concept_emb = nn.Parameter(torch.randn(self.concept_num, emb_size).to(device), requires_grad=True)#知识点表征
    ...
    def obtain_v(self, q, c, h, x, emb):
        ...
        predict_x = torch.cat([h, v], dim = 1).to(device)#equation4
        h_v = torch.cat([h, v], dim = 1).to(device) #equation4 为啥要计算两次？
        prob = self.predictor(torch.cat([
            predict_x, emb
        ], dim = 1)).to(device)#equation7
        return h_v, v, prob, x
    ...
    def update_state(self, h, v, emb, operate):
        ...
        v_cat = torch.cat([
            ...
            v.mul((1 - operate).repeat(1, self.emb_size * 2))], dim = 1).to(device)#v_t扩展，分别对应正确的错误的情况
        e_cat = torch.cat([
            emb.mul((1-operate).repeat(1, self.emb_size * 2)),
            emb.mul((operate).repeat(1, self.emb_size * 2))], dim = 1).to(device)# s_t 扩展，分别对应正确的错误的情况
...
class IEKT(QueBaseModel):
    ...
    def train_one_step(self,data,process=True):
        ...
        for i in range(0, data_len):
            ...
            this_cog_state = torch.cat([
                ...
                torch.zeros(1, pre_state_tensor[i][0].size()[0]).to(device)
            ], dim=0)
            this_sens_state = torch.cat([
                state_tensor[i][0: this_seq_len],
                torch.zeros(1, state_tensor[i][0].size()[0]).to(device)
            ], dim=0)
            ...
            advantage_cog = torch.tensor(advantage_lst_cog, dtype=torch.float).to(device)
            ...
            advantage_sens = torch.tensor(advantage_lst_sens, dtype=torch.float).to(device)
    ...
    def predict_one_step(self,data,return_details=False,process=True):
        ...
        # print(data_new.is_cuda)
        data_new['cc'] = data_new['cc'].to(device)
        data_new['cq'] = data_new['cq'].to(device)
        data_new['cr'] = data_new['cr'].to(device)
        # data_new = data
        data_len = data_new['cc'].shape[0]
        seq_len = data_new['cc'].shape[1]
        h = torch.zeros(data_len, self.model.emb_size).to(device)
        ...
        rt_x = torch.zeros(data_len, 1, self.model.emb_size * 2).to(device)
        for seqi in range(0, seq_len):#序列长度
            ...
            out_operate_logits = torch.where(prob > 0.5, torch.tensor(1).to(device), torch.tensor(0).to(device)) 
            ...
            this_reward = torch.where(out_operate_logits.squeeze(1).float() == ground_truth,
                            ...
                            torch.tensor(0).to(device))# if condition x else y,这里相当于统计了正确的数量
