...
def compute_phi_corr(df, dpath):
    ...
    level = "questions"
    if "usetimes" in keys:
        df_ = df.explode(['questions','concepts','responses','timestamps','usetimes'])
    elif 'timestamps' in keys and 'questions' in keys:
        ...
        if 'timestamps' in keys:
            ...
            df_ = df.explode(['concepts','responses'])
    else:
    ...
    for fold in all_folds:
        tmp_folds = all_folds[:]
        tmp_folds.remove(fold) 
        folds_str = "_" + "_".join([str(_) for _ in tmp_folds])
        
        df2_ = df_[df_['fold'].isin(tmp_folds)] 
        print(df2_)    
        print(f"fold:{fold} total user num : {df2_['uid'].nunique()}")
        ...
        for stu,stu_df in df2_.groupby(['uid'], sort=False):
            count+=1

            question = np.array( [int(q) for q in stu_df[level].values] )
            qu = np.array( [int(q) for q in stu_df[level].unique()] )
        ...
        pd.to_pickle( phi_dict, os.path.join(dpath, fname) )  
        print("\n")
