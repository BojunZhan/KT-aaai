...
def get_sub_dataset_quelevel(data_config, train_ratio=1.0):
    df = pd.read_csv(
...
def get_sub_dataset(data_config, train_ratio=1.0):
    df = pd.read_csv(
        os.path.join(data_config["dpath"], f"train_valid_sequences.csv")
        )
    ins, ss, qs, cs, seqnum = calStatistics(
        df=df, stares=[], key="original train+valid KC level"
    )
    print(
        f"origin interactions num: {ins}, select num: {ss}, qs: {qs}, cs: {cs}, seqnum: {seqnum}"
    )
    if train_ratio < 1.0:
        ...
        if not os.path.exists(sub_data_path):
            ...
            finaldf = pd.read_csv(sub_data_path)
        ins, ss, qs, cs, seqnum = calStatistics(
            df=finaldf, stares=[], key="original train+valid KC level"
        )   
        print(
            f"after extract interactions num: {ins}, select num: {ss}, qs: {qs}, cs: {cs}, seqnum: {seqnum}"
        )
    ...
    if train_ratio < 1.0:
        sub_sequence_df.to_csv(
            f"{dpath}/train_valid_sequences_sequences_{train_ratio}.csv",
            index=None,
        )
    else:
        print("do not have ratio")
